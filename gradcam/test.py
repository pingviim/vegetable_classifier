...
from config import DEVICE, BASE_SAVE_DIR
...
GRADCAM_DIR = os.path.dirname(os.path.abspath(__file__))
RESULTS_DIR = os.path.join(GRADCAM_DIR, 'results')
EXAMPLES_DIR = os.path.join(GRADCAM_DIR, 'examples')


def get_test_loader():
    test_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'training', 'dataset', 'test')

    if not os.path.exists(test_dir):
        print(f"Test directory not found: {test_dir}")
        print("Please run: python gradcam/download_dataset_for_test.py")
...
def analyze_heatmaps_by_class(model, model_type, dataloader, class_names, device, num_per_class=5):
    print("\n" + "=" * 60)
    print("HEATMAP ANALYSIS BY CLASS")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("MODEL ATTENTION STATISTICS BY CLASS")
    print("=" * 60)
    print(f"{'Class':<20} {'Center':<10} {'Edge':<10} {'Texture':<10}")
    print("-" * 50)
...
def visualize_class_heatmaps(class_heatmaps, class_names, num_examples=5):
    ...
    for group_idx in range(n_groups):
        ...
        plt.suptitle(f'Grad-CAM Heatmaps by Class (Group {group_idx + 1}/{n_groups})',
                     fontsize=14, y=1.02)
        plt.tight_layout()
        plt.savefig(os.path.join(RESULTS_DIR, f'heatmaps_by_class_group_{group_idx + 1}.png'),
                    dpi=150, bbox_inches='tight')
        plt.close()

        print(f"Saved group {group_idx + 1}/{n_groups}: classes {start_idx + 1}-{end_idx}")
...
def visualize_decisions_comparison(correct_preds, wrong_preds, num_show=5):
    ...
    plt.suptitle('Decision Comparison: Correct (green) vs Wrong (red)', fontsize=14)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, 'decisions_comparison.png'), dpi=150, bbox_inches='tight')
    plt.close()


def main():
    print("\n" + "=" * 60)
    print("GRAD-CAM ANALYSIS AND MODEL INTERPRETATION")
    print("=" * 60)
    print(f"Results will be saved to: {RESULTS_DIR}")
    print(f"Examples will be saved to: {EXAMPLES_DIR}")

    print("\nLoading model...")
    model_path = os.path.join(BASE_SAVE_DIR, MODEL_TYPE, "best_model.pth")
    ...
    print("\n2. Heatmap analysis for different classes...")
    ...
    save_gradcam_examples(model, MODEL_TYPE, test_loader,
                          ...
                          save_dir=EXAMPLES_DIR, num_examples=10)
    ...
    for i in range(min(3, len(test_dataset))):
        ...
        visualize_single_image(model, MODEL_TYPE, image_tensor, image_pil,
                               test_dataset.classes, save_path=os.path.join(RESULTS_DIR, f'analysis_{i + 1}.png'))

        visualize_top_classes(model, MODEL_TYPE, image_tensor, image_pil,
                              test_dataset.classes, top_k=3, save_path=os.path.join(RESULTS_DIR, f'top3_{i + 1}.png'))

    print("\n" + "=" * 60)
    print("ANALYSIS COMPLETE")
    print(f"Results saved in folders:")
    print(f"   * {RESULTS_DIR}/ - visualizations")
    print(f"   * {EXAMPLES_DIR}/ - gradcam examples")
    print("Reports include:")
    print("   * heatmaps_by_class_group_*.png - heatmap visualization by class")
    print("   * decisions_comparison.png - correct/wrong decision comparison")
    print("   * Model attention statistics")
